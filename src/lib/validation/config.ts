...
import { AppErrorClass, ErrorType, logError } from "../api/error-handling";
...
// Utilisation des URLs AgentConnect qui fonctionnent actuellement
...
export function validateProConnectEnvironment(): ProConnectValidationResult {
  ...
  if (issuer) {
    ...
    if (issuer !== expectedEndpoints.issuer) {
      errors.push(
        `PROCONNECT_ISSUER incorrect pour l'environnement ${environment}. ` +
          `Attendu: ${expectedEndpoints.issuer}, Reçu: ${issuer}`
      );
    }
  } else {
    // Si pas d'issuer défini, utiliser celui par défaut selon l'environnement
    const expectedEndpoints =
      PROCONNECT_ENDPOINTS[
        environment === "development" ? "integration" : environment
      ];
    warnings.push(
      `PROCONNECT_ISSUER non défini. Utilisation par défaut: ${expectedEndpoints.issuer}`
    );
  }
...
export function validateConfigurationAtStartup(): void {
  ...
  if (isBuildTime) {
    console.log("⏭️  Validation de configuration ignorée pendant le build");
    return;
  }
...
export function validateRuntimeConfiguration(): void {
  ...
  if (!result.isValid) {
    // Détecter si nous sommes dans un contexte de build ou de développement
    const isBuildTime =
      process.env.NEXT_PHASE === "phase-production-build" ||
      process.env.npm_lifecycle_event === "build" ||
      process.argv.includes("build");

    // En développement ou lors du build, permettre de continuer avec des warnings
    if (process.env.NODE_ENV !== "production" || isBuildTime) {
      console.warn(
        "⚠️  Configuration ProConnect invalide en développement/build:",
        result.errors.join(", ")
      );
      return;
    }
